import { createSeriesFromForm } from "@/app/actions/series";
    ...
    <div className="mx-auto max-w-3xl space-y-8 px-4 py-10 sm:px-6">
      ...
      {ready && !user && (
        <div className="app-panel px-6 py-8 text-center text-sm text-[var(--md-text-muted)]">
          <p className="mb-4">
            Sign in to submit a new series. Every submission is queued for admin
            approval before it appears in the public catalogue.
          </p>
          <NavLinkButton href="/login?next=/series/new" className="px-5 py-2.5">
            Sign in to continue
          </NavLinkButton>
        </div>
      )}
      ...
      {ready && user && (
        <div className="space-y-8">
          <p className="mdc-moderation-pending rounded-2xl px-4 py-3 text-sm">
            New series stay <span className="font-semibold">pending</span> until
            an admin approves them. They won&apos;t show on the series page or to
            other members until then.
          </p>
          <SeriesForm action={createSeriesFromForm} />
        </div>
      )}
    </div>
  );
