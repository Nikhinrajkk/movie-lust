...
3. **Database**
   ...
   - `supabase/migrations/0005_imdb_ls000047280_serial_investigation.sql` — optional extra seed titles (idempotent; disables the insert trigger briefly if present).
   - `supabase/migrations/0006_movie_watch_provider.sql` — adds nullable `movies.watch_provider` (OTT slug for “where to watch” in the UI).
...
For production hosting (e.g. **Vercel**), set `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` in the host’s environment.

**Auth on localhost and production together:** In **Supabase → Authentication → URL configuration**, **Redirect URLs** is a list of allowed return addresses. Add **one line per origin** (do not delete localhost when you add Vercel). Typical entries:

- `http://localhost:4000/auth/callback` — matches `npm run dev` (see `package.json` port).
- `https://your-app.vercel.app/auth/callback` — your deployed app.

If you sometimes use another dev port (e.g. `3000`), add that URL too. If an origin is missing from this list, Supabase ignores the app’s `redirectTo` and sends the user to **Site URL** instead (wrong host / broken session). **Site URL** can stay on production or localhost; it is only the default when no allowed redirect matches.

Google sign-in and magic links use the **browser’s current origin** for the return URL, so you do not need `NEXT_PUBLIC_SITE_URL` for auth. If you set it on Vercel, do not use `http://localhost:…` there for Production.
